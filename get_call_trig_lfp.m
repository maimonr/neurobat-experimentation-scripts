...
for call_k = 1:length(used_call_idx)
    
    current_call_k = used_call_idx(call_k);
    [~,csc_call_idx] = min(abs(timestamps - callpos(1,current_call_k)));
    csc_idx = (csc_call_idx-lfp_call_offset_csc_samples):(csc_call_idx+lfp_call_offset_csc_samples);
    call_lfp_data = lfpData.lfpData(:,csc_idx);
    
    for ch = 1:n_channel
        call_trig_csc(:,call_k,ch) = call_lfp_data(ch,:);
        call_trig_csc(:,call_k,ch) = filtfilt(notch_filter_60Hz,call_trig_csc(:,call_k,ch));
        call_trig_csc(:,call_k,ch) = filtfilt(notch_filter_120Hz,call_trig_csc(:,call_k,ch));
    end
end
...
parsave(outDir,call_trig_csc_struct);
